# GeoClue 2 daemon with debug flag.
# for info about debugging, refer to https://gitlab.freedesktop.org/geoclue/geoclue/-/blob/master/HACKING.md?ref_type=heads
# I have nmea socket enabled as well.
{
  config,
  lib,
  pkgs,
...
let
  cfg = config.services.mygeoclue2;
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.geoclue = {
      ...
      serviceConfig = {
        ...
        User = "geoclue";
        Environment = "\"GSETTINGS_BACKEND=memory\" \"G_MESSAGES_DEBUG=Geoclue\"";
        ExecStart = "${cfg.package}/libexec/geoclue";
